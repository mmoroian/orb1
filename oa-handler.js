document.addEventListener("DOMContentLoaded", () => { // Loader function
  ...
  const companyAddressInput = document.getElementById("oa_company_address"); // <— NEW for distance calc
  ...
  window.loadCompanyByName = async () => {
    ...
    try {
      ...
      setVal("oa_company_address", found.company_address || "");  // <— NEW for distance calc
      ...
      // --- Trucking to Toronto (array on the company object) ---
      const tt = Array.isArray(found.trucking_to_toronto) ? found.trucking_to_toronto : [];
      for (let i = 1; i <= 3; i++) {
        const row = tt[i - 1] || {};
        setVal(`trucking_min${i}`, row.min || "");
        setVal(`trucking_max${i}`, row.max || "");
        setVal(`trucking_rate${i}`, row.rate || "");
        setRadioVal(`trucking_type_${i}`, row.type || "");
      }
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const data = {
      company_name,
        company_address: getVal("oa_company_address"),  // <— NEW for distance calc
    ...
    // --- Trucking to Toronto ---
    data.trucking_to_toronto = [];
    for (let i = 1; i <= 3; i++) {
      const row = {
        min: getVal(`trucking_min${i}`),
        max: getVal(`trucking_max${i}`),
        rate: getVal(`trucking_rate${i}`),
        type: getRadioVal(`trucking_type_${i}`) // "variable" | "flat"
      };
      // push only if something is filled
      if (row.min || row.max || row.rate || row.type) data.trucking_to_toronto.push(row);
    }
